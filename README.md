...
Load your `.env`, then start the receiver:
...
- `POST /webhooks/github` → verifies the GitHub HMAC signature, filters for
  merged pull-request events, and persists the delivery to the SQLite store at
  `${LIVING_ADR_STORAGE_PATH}/ingestion.db`.
...
The public URL is stable across restarts and has the form:
...
Both the receiver (`living-adr-workflow`) and the tunnel host
(`devtunnel host living-adr`) must stay running. The tunnel URL is persistent —
re-running both restores the same URL.
...
The webhook receiver currently does **receive → verify → filter → persist**:

- ✅ Receives GitHub deliveries over the tunnel.
- ✅ Verifies the HMAC signature against `GITHUB_WEBHOOK_SECRET`.
- ✅ Filters for merged pull-request events; deduplicates by delivery id.
- ✅ Persists every delivery to the SQLite ingestion store.

It does **not** yet act on pull requests — the receiver runs with no SCM
provider, so the evidence-fetch → ADR-draft → review → publish chain is not
wired into the running service. Enabling it requires wiring a GitHub App provider
(built from `GITHUB_APP_ID`, the installation id, and the `.pem`) into the
workflow startup, plus the `ANTHROPIC_API_KEY` / `LANGSMITH_API_KEY` for the
draft and observability stages.
